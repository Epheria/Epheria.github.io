...
게임 엔진에서 렌더링 파이프라인의 첫 단계가 **카메라가 보는 영역을 결정하는 것**이듯, 인터넷 통신의 첫 단계는 여러분의 기기에서 가장 가까운 네트워크 장비까지 데이터를 보내는 것입니다. 이 구간을 통신 업계에서는 **"라스트 마일(Last Mile)"**이라고 부릅니다. 실제로는 마지막이 아니라 **첫 번째** 구간이지만, 통신사 입장에서 가입자에게 도달하는 "마지막 구간"이라는 의미입니다.
...
| 대역 | 주파수 | 채널 수 | 특징 | 비유 |
|------|--------|---------|------|------|
| 2.4 GHz | 2.400~2.4835 GHz | 14개 (한국 13개) | 넓은 범위, 벽 투과력 우수, 느림 | 저해상도 텍스처 — 넓은 영역 커버 |
| 5 GHz | 5.150~5.850 GHz | 25개+ (DFS 포함) | 좁은 범위, 빠름, 벽에 약함 | 고해상도 텍스처 — 가까이서 선명 |
| 6 GHz (WiFi 6E/7) | 5.925~7.125 GHz | 59개 | 매우 빠름, 매우 좁은 범위 | 4K 텍스처 — 바로 눈앞에서만 유효 |

게임 개발의 비유로 설명하면, WiFi 채널은 **GPU의 렌더 큐(Render Queue)**와 같습니다. 같은 주파수 채널을 여러 기기가 공유하면 "리소스 충돌"이 발생하고, 렌더링 파이프라인에서 드로콜이 겹치면 성능이 떨어지듯 WiFi도 간섭(Interference)으로 속도가 저하됩니다.
...
게임으로 비유하면, 광케이블은 **전용 서버(Dedicated Server)**, WiFi는 **P2P 매치메이킹**, 셀룰러는 **클라우드 게이밍**, 위성은 **크로스 리전 플레이**와 비슷합니다. 각각의 트레이드오프가 명확하고, 용도에 따라 선택이 달라집니다.

---

## Part 2: 해저 광케이블 — 빛의 대동맥

게임 개발에서 대규모 에셋 스트리밍을 할 때, 데이터가 디스크에서 메모리로, 메모리에서 GPU VRAM으로 이동하는 **버스(Bus) 대역폭**이 병목이 되듯이, 전 세계 인터넷에서 대륙 간 데이터 이동의 대동맥 역할을 하는 것이 바로 **해저 광케이블**입니다.
...
게임 개발자에게 친숙한 비유로 설명하면, **싱글모드는 단일 레이캐스트(Raycast)**와 같습니다. 빛이 하나의 경로로 직진하므로, 신호가 멀리 가도 퍼지지 않아 선명합니다. **멀티모드는 SphereCast나 산란광(Scatter)** 과 같습니다. 빛이 여러 경로로 반사되면서 진행하기 때문에, 각 경로의 도착 시간이 미세하게 달라져 **모드 분산(Modal Dispersion)**이 발생합니다. 이 분산이 장거리에서 신호를 뭉개뜨리므로, 멀티모드는 단거리에만 사용합니다.
...
게임으로 비유하면, 오디오 시스템에서 사운드 소스(AudioSource)의 거리 감쇠(Attenuation)가 있을 때, 중간에 **리피터(Repeater)**를 두어 볼륨을 다시 올리는 것과 유사합니다. 다만 리피터를 너무 많이 거치면 잡음도 함께 증폭되므로, 원본 신호의 품질을 유지하기 위한 에러 정정이 필요합니다.
...
게임 개발의 비유로 설명하면, **멀티플랫폼 게임 서버**가 하나의 데이터센터 안에서 여러 ISP와 직접 연결되는 것과 같습니다. PC, 콘솔, 모바일 유저가 서로 다른 네트워크를 쓰더라도, 게임 서버가 여러 ISP의 피어링 포인트에 직접 접속해 있으면 모든 유저에게 낮은 지연시간을 제공할 수 있습니다.
...
게임 개발의 비유로 하면, **라이브 서비스(LiveOps)에서 서버를 끄지 않고 핫픽스(Hotfix)를 배포**하는 것과 같습니다. Unity의 Addressables에서 에셋 번들을 런타임에 교체하는 것, Unreal의 핫 리로드(Hot Reload)로 코드를 수정하는 것과 비슷한 철학입니다. 핵심은 **서비스 중단 없이 구성요소를 교체**하는 것입니다.
...
데이터센터 기술자는 게임 서버의 **라이브 옵스(LiveOps) 엔지니어**와 본질적으로 같은 일을 합니다. 라이브 서비스에서 서버를 끄지 않고 패치하고, 장애를 모니터링하며, 트래픽 폭주에 대응하고, 하드웨어 교체를 수행합니다. 차이가 있다면, LiveOps 엔지니어는 소프트웨어를 다루고, 데이터센터 기술자는 소프트웨어와 하드웨어를 모두 다룬다는 것입니다.
...
게임 개발자에게 친숙한 비유로 정리하면 이렇습니다.

- **SSD** = **RAM의 느린 버전**. 빠르고 편리하지만, 전원이 꺼지면 데이터가 서서히 증발합니다. DRAM이 밀리초 단위로 사라진다면, SSD(NAND Flash)는 수개월~수년 단위로 사라집니다.
- **HDD** = **디스크에 물리적으로 새긴 세이브 파일**. 느리지만 전원 없이도 오래 보존됩니다. 물리적 충격에만 주의하면 됩니다.
- **자기 테이프** = **아카이브에 봉인된 마스터 빌드**. 접근 속도는 가장 느리지만 보존 기간은 가장 깁니다.
