...
tldr:
  ...
  - "일론 머스크는 '에너지가 진짜 화폐'라 주장하며, 비트코인을 에너지를 가치로 변환하는 '물리학 기반 화폐'로 정의했다"
  - "구글 윌로우(105큐비트)는 비트코인을 깨려면 수백만 큐비트가 필요한 현실과 비교하면 아직 먼 거리이며, 양자 위협이 현실화되면 비트코인보다 인류 디지털 인프라 전체가 먼저 위기에 처한다"
...
### 에너지가 화폐가 된다 — 일론 머스크의 예측

2025년 말, 일론 머스크는 인도 기업가 니킬 카마스(Nikhil Kamath)와의 팟캐스트에서 흥미로운 예측을 했다.

> **"에너지가 진짜 화폐다(Energy is the real currency). 정부는 돈을 찍어낼 수 있지만, 에너지를 찍어낼 수는 없다."**

머스크는 AI와 로봇 기술이 발전하면 인간 노동의 가치가 근본적으로 변하고, **"돈이라는 개념 자체가 사라질 것"**이라 주장했다. 그리고 그 세계에서 가치의 진정한 척도는 **에너지**가 될 것이라고 했다. 비트코인을 **"물리학 기반 화폐(Physics-based Currency)"**라 부른 이유가 여기에 있다.

왜 에너지인가? 물리학의 관점에서 보면, 우주에서 모든 가치 있는 작업(연산, 이동, 생산, 통신)은 결국 **에너지 변환**이다. 금을 캐는 것도, 반도체를 만드는 것도, 서버를 돌리는 것도 에너지가 필요하다. 에너지는 법률로 만들 수 없고, 정치적으로 조작할 수 없다. **열역학 법칙을 회피할 수 있는 정부는 없다.**

이것을 비트코인의 맥락에서 보면 논리가 연결된다:

```
[법정화폐 vs 비트코인: 에너지 관점]

법정화폐:
  가치의 근거 = 정부의 신용 (추상적, 변동 가능)
  발행 비용  = 종이 + 잉크 (거의 0에 수렴)
  조작 가능성 = 높음 (양적 완화, 금리 조절 등)

비트코인:
  가치의 근거 = 투입된 에너지 + 수학적 희소성 (물리적, 불변)
  발행 비용  = 전기 에너지 + 컴퓨팅 파워 (실제 물리적 비용)
  조작 가능성 = 없음 (열역학 법칙을 바꿀 수 없다)
```

1편에서 살펴본 것처럼, 법정화폐의 가치는 "정부가 가치가 있다고 말하기 때문에" 존재한다. 반면 비트코인의 가치에는 **실제 에너지가 응축**되어 있다. 1 BTC를 채굴하는 데 현재 약 **266,000 kWh**의 전기가 소모된다. 이것은 미국 가정 약 9년치 전기 사용량이다. 이 에너지는 회수할 수 없다. 되돌릴 수 없다. SHA-256 해시에 영원히 "소각"된 것이다.

NVIDIA의 CEO 젠슨 황(Jensen Huang)도 비슷한 견해를 밝힌 바 있다. AI 시대에 **컴퓨팅 파워 = 에너지 = 가치**라는 등식이 점점 더 명확해지고 있다는 것이다.

물론 반론도 있다. 에너지를 소비했다는 사실 자체가 가치를 만드는 것은 아니다. 땅에 구덩이를 파고 다시 메우는 데 에너지를 쓴다고 해서 가치가 생기지 않는다. 비트코인의 에너지 소비가 가치를 가지는 이유는, 그 에너지가 **네트워크 보안을 유지하는 작업증명**에 사용되기 때문이다. 비트코인에 투입된 에너지는 "변조 불가능한 글로벌 장부"라는 서비스를 유지하는 비용이다.

앞으로 에너지 생산 비용이 극적으로 떨어진다면 어떨까? (핵융합 발전 등) 채굴 비용이 감소하면 더 많은 채굴자가 참여하고, 난이도가 올라가며, 다시 균형이 잡힌다. 비트코인의 난이도 조절 메커니즘이 이 상황도 흡수한다. **에너지가 싸지면 → 채굴자 증가 → 난이도 상승 → 블록당 에너지 소비 증가 → 다시 균형.** 비트코인은 사용 가능한 에너지를 끊임없이 흡수하도록 설계된 시스템이다.

머스크의 예측이 맞든 틀리든, 한 가지는 분명하다. 비트코인은 **디지털 세계에서 에너지를 가치로 변환하는 메커니즘**이라는 독특한 위치를 점하고 있다. 이것은 다른 어떤 암호화폐(PoS 기반)나 법정화폐와도 근본적으로 다른 특성이다.

### 양자 컴퓨팅의 위협 — 비트코인의 종말 시나리오?

"양자 컴퓨터가 비트코인을 깰 수 있지 않나?" — 암호화폐에 관심이 있다면 한 번쯤 들어본 질문일 것이다. 2024년 12월 구글이 **105큐비트 양자칩 '윌로우(Willow)'**를 발표하면서 이 질문이 다시 뜨거워졌다.

윌로우는 기존 슈퍼컴퓨터로 **10셉틸리온 년**(10,000,000,000,000,000,000,000,000년)이 걸리는 계산을 **5분 만에** 수행했다. 이런 숫자를 들으면 "비트코인의 수조 년 걸리는 해시 계산도 초 단위로 깨지는 거 아닌가?"라고 생각할 수 있다. 하지만 현실은 그렇게 단순하지 않다.

#### 양자 컴퓨터가 비트코인을 공격하는 두 가지 경로

비트코인은 두 가지 암호학 체계에 의존하고, 각각에 대한 양자 위협의 수준이 다르다:

| 암호 체계 | 용도 | 양자 공격 알고리즘 | 위협 수준 |
|-----------|------|-------------------|----------|
| **ECDSA (secp256k1)** | 거래 서명 (개인 키 → 공개 키) | **쇼어 알고리즘** (지수적 가속) | **높음** — 공개 키에서 개인 키 역추출 가능 |
| **SHA-256** | 채굴 (작업증명), 블록 연결 | **그로버 알고리즘** (제곱근 가속) | **낮음** — $2^{256}$이 $2^{128}$로 줄어드는 수준 |

이 차이가 핵심이다. 양자 컴퓨터의 위협은 **균일하지 않다.**

**쇼어 알고리즘과 ECDSA**: 가장 심각한 위협은 거래 서명 체계에 대한 것이다. 현재 비트코인은 ECDSA를 사용하여 "이 거래가 비트코인 소유자에 의해 승인되었다"를 증명한다. 쇼어 알고리즘은 이 과정을 역전시킬 수 있다 — 공개 키만으로 개인 키를 추출하는 것. 성공하면 **다른 사람의 비트코인을 마음대로 사용**할 수 있게 된다.

**그로버 알고리즘과 SHA-256**: 채굴에 사용되는 SHA-256에 대해서는 그로버 알고리즘이 적용될 수 있지만, 이는 **제곱근 수준의 가속**에 불과하다. $2^{256}$번의 시도가 $2^{128}$번으로 줄어드는 것이다. $2^{128}$이 얼마나 큰 수인지 감이 안 온다면 — 이것은 **340,282,366,920,938,463,463,374,607,431,768,211,456**이다. 여전히 브루트포스로 깨기에는 천문학적인 숫자다. SHA-256은 양자 컴퓨터에 대해서도 사실상 안전하다.

#### 현실과의 거리: 105큐비트 vs 수백만 큐비트

그렇다면 비트코인의 ECDSA를 깨려면 얼마나 강력한 양자 컴퓨터가 필요한가?

```
[양자 컴퓨터의 현재 vs 비트코인을 깨는 데 필요한 수준]
...
ECDSA 해독에 필요한 수준:
  약 2,330 논리적 큐비트 (최소 이론값)
  → 오류 보정을 감안하면: 수백만 물리적 큐비트 필요

SHA-256 해독에 필요한 수준:
  약 250만 논리적 큐비트
  → 오류 보정을 감안하면: 수천만~수억 물리적 큐비트 필요

현재 갭:
  105 큐비트 → 수백만 큐비트
  약 10,000배 ~ 1,000,000배의 격차
```

여기서 "논리적 큐비트"와 "물리적 큐비트"의 차이가 중요하다. 양자 컴퓨터는 극도로 불안정하다. 큐비트는 주변 환경의 미세한 진동, 온도 변화, 전자기파에 의해 쉽게 오류가 발생한다 (이를 **디코히어런스**라 한다). 하나의 안정적인 "논리적 큐비트"를 만들려면 수천 개의 물리적 큐비트로 오류를 보정해야 한다.

게임 개발자라면 이런 비유가 와닿을 것이다. **물리 시뮬레이션의 정밀도**를 생각해보자. `float` 정밀도로 충분한 작업이 있고, `double`이 필요한 작업이 있다. 양자 컴퓨터도 마찬가지로, 단순한 계산(오류가 좀 있어도 되는)과 암호 해독(완벽한 정확도가 필요한)에서 요구하는 큐비트 수가 완전히 다르다.

2026년 2월 기준, Iceberg Quantum의 **피너클 아키텍처(Pinnacle Architecture)**가 QLDPC 코드를 사용하여 RSA-2048 해독에 필요한 물리적 큐비트를 10만 개 미만으로 줄이는 데 성공했다. 기존 대비 약 10배 감소다. 양자 컴퓨팅의 효율이 빠르게 개선되고 있다는 신호이지만, 비트코인을 깨기에는 여전히 먼 길이다.

Blockstream CEO 아담 백(Adam Back)은 "암호학적으로 의미 있는 양자 위협은 **20~40년 뒤**에나 현실이 될 것"이라는 입장이다.

#### 만약 양자 컴퓨터가 정말 비트코인을 깬다면?

여기서 더 큰 그림을 봐야 한다. 비트코인의 ECDSA를 깰 수 있는 양자 컴퓨터가 등장한다는 것은, **비트코인만의 문제가 아니다.**

```
[양자 컴퓨터가 ECDSA를 깨면 함께 무너지는 것들]
...
→ 인류 문명의 디지털 인프라 전체가 위협받는 상황
→ 이때 "비트코인이 위험하다"는 것은 "집에 불이 났는데 액자가 비뚤어졌다"고 걱정하는 수준
```

따라서 양자 컴퓨터가 현실적 위협이 되는 시점에는, 비트코인뿐 아니라 **인류의 모든 디지털 보안 체계가 양자 내성(Post-Quantum) 암호로 전환**하고 있을 것이다. 그리고 비트코인도 예외가 아니다.

#### 비트코인의 방어선: 양자 내성 암호

비트코인은 **소프트포크(Soft Fork)**를 통해 프로토콜을 업그레이드할 수 있다. 실제로 양자 내성 서명 방식으로의 전환은 이미 연구되고 있다:

- **NIST 표준화**: 2024년 미국 국립표준기술연구소(NIST)가 양자 내성 암호 표준 3가지를 발표 (CRYSTALS-Kyber, CRYSTALS-Dilithium, SPHINCS+)
- **비트코인 커뮤니티**: BIP(Bitcoin Improvement Proposal)를 통한 양자 내성 서명 알고리즘 도입이 논의 중
- **해시 기반 서명**: SHA-256 자체가 양자 내성이 높으므로, Lamport 서명 같은 해시 기반 서명으로 전환하면 추가적인 안전성 확보 가능

비트코인이 가진 또 하나의 방어 메커니즘은 **주소 재사용 회피**다. 비트코인 주소는 공개 키의 해시인데, 거래를 보내기 전까지는 공개 키 자체가 네트워크에 노출되지 않는다. 양자 컴퓨터의 쇼어 알고리즘은 **공개 키**를 알아야 개인 키를 추출할 수 있으므로, 한 번도 사용하지 않은 주소에 보관된 비트코인은 양자 컴퓨터로부터 안전하다.

즉, 양자 위협에 대한 현실적 평가는 이렇다:

| 시나리오 | 확률 | 비트코인 영향 |
|----------|------|-------------|
| 10년 내 암호학적 양자 컴퓨터 등장 | 매우 낮음 | 전환 시간 충분 |
| 20~30년 내 등장 | 가능성 있음 | 양자 내성 암호로 업그레이드 완료 예상 |
| 양자 컴퓨터가 갑자기 비밀리에 개발됨 | 극히 낮음 | 모든 디지털 보안이 동시 위협 |
| 양자 컴퓨터가 영원히 실용화 안 됨 | 가능성 있음 | 비트코인에 영향 없음 |

결론적으로, 양자 컴퓨터는 비트코인의 **이론적 위협**이지만 **즉각적인 위험**은 아니다. 비트코인은 15년간의 역사에서 수많은 기술적 도전을 극복하며 업그레이드되어 왔고, 양자 위협에 대한 대비도 이미 진행 중이다. 단, 기술의 발전 속도는 예측하기 어렵다는 점은 항상 염두에 둘 필요가 있다.
