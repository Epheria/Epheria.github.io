...
Tick Rate 64  = 15.6ms마다 업데이트 (경쟁 FPS — 발로란트)
Tick Rate 128 = 7.8ms마다 업데이트  (하드코어 FPS — CS2)
```
...
데디케이티드 서버가 치트 방지에 강한 이유가 바로 이것입니다. 게임의 모든 중요한 판정(히트 판정, 데미지 계산, 아이템 획득 등)이 서버에서 이루어지기 때문에, 클라이언트를 아무리 해킹해도 서버를 속일 수 없습니다.
