...
tldr:
  ...
  - "イーロン・マスクは「エネルギーが本当の通貨」と主張し、ビットコインをエネルギーを価値に変換する「物理学基盤通貨」と定義した"
  - "グーグルのウィロー(105キュービット)はビットコインを破るのに必要な数百万キュービットとは程遠く、量子脅威が現実化すればビットコインより人類のデジタルインフラ全体が先に危機に瀕する"
...
### エネルギーが通貨になる — イーロン・マスクの予測

2025年末、イーロン・マスクはインドの起業家ニキル・カマス(Nikhil Kamath)とのポッドキャストで興味深い予測をした。

> **「エネルギーが本当の通貨だ(Energy is the real currency)。政府はお金を刷れるが、エネルギーを刷ることはできない。」**

マスクはAIとロボット技術が発展すれば人間の労働の価値が根本的に変わり、**「お金という概念自体が消えるだろう」** と主張した。そしてその世界で価値の真の尺度は **エネルギー** になるだろうと言った。ビットコインを **「物理学基盤通貨(Physics-based Currency)」** と呼んだ理由がここにある。

なぜエネルギーか？ 物理学の観点から見れば、宇宙ですべての価値ある作業（演算、移動、生産、通信）は結局 **エネルギー変換** だ。金を掘ることも、半導体を作ることも、サーバーを回すこともエネルギーが必要だ。エネルギーは法律で作れないし、政治的に操作できない。**熱力学法則を回避できる政府はない。**

これをビットコインの文脈で見れば論理がつながる：

```
[法定通貨 vs ビットコイン：エネルギー観点]

法定通貨:
  価値の根拠 = 政府の信用 (抽象的、変動可能)
  発行費用  = 紙 + インク (ほぼ0に収束)
  操作可能性 = 高い (量的緩和、金利調節など)

ビットコイン:
  価値の根拠 = 投入されたエネルギー + 数学的希少性 (物理的、不変)
  発行費用  = 電気エネルギー + コンピューティングパワー (実際の物理的費用)
  操作可能性 = なし (熱力学法則を変えることはできない)
```

1編で調べたように、法定通貨の価値は「政府が価値があると言うから」存在する。一方ビットコインの価値には **実際のエネルギーが凝縮** されている。1 BTCを採掘するのに現在約 **266,000 kWh** の電気が消費される。これはアメリカの家庭の約9年分の電気使用量だ。このエネルギーは回収できない。元に戻せない。SHA-256ハッシュに永遠に「焼却」されたのだ。

NVIDIAのCEOジェンスン・ファン(Jensen Huang)も似た見解を明らかにしたことがある。AI時代に **コンピューティングパワー = エネルギー = 価値** という等式がますます明確になっているということだ。

もちろん反論もある。エネルギーを消費したという事実自体が価値を作るわけではない。地面に穴を掘って再び埋めるのにエネルギーを使っても価値は生まれない。ビットコインのエネルギー消費が価値を持つ理由は、そのエネルギーが **ネットワークセキュリティを維持する作業証明** に使用されるからだ。ビットコインに投入されたエネルギーは「変調不可能なグローバル帳簿」というサービスを維持する費用だ。

今後エネルギー生産費用が劇的に下がればどうなるか？（核融合発電など）採掘費用が減少すればより多くの採掘者が参加し、難易度が上がり、再び均衡が取れる。ビットコインの難易度調節メカニズムがこの状況も吸収する。**エネルギーが安くなれば → 採掘者増加 → 難易度上昇 → ブロックあたりエネルギー消費増加 → 再び均衡。** ビットコインは使用可能なエネルギーを絶えず吸収するように設計されたシステムだ。

マスクの予測が当たろうと外れようと、一つは明確だ。ビットコインは **デジタル世界でエネルギーを価値に変換するメカニズム** という独特な位置を占めている。これは他のどんな暗号通貨（PoS基盤）や法定通貨とも根本的に異なる特性だ。

### 量子コンピューティングの脅威 — ビットコインの終末シナリオ？

「量子コンピュータがビットコインを破れるのではないか？」 — 暗号通貨に関心があれば一度は聞いたことがある質問だろう。2024年12月グーグルが **105キュービットの量子チップ「ウィロー(Willow)」** を発表してこの質問が再び熱くなった。

ウィローは既存のスーパーコンピュータで **10セプティリオン年**（10,000,000,000,000,000,000,000,000年）かかる計算を **5分で** 実行した。こんな数字を聞けば「ビットコインの数兆年かかるハッシュ計算も秒単位で破れるのでは？」と思うかもしれない。しかし現実はそう単純ではない。

#### 量子コンピュータがビットコインを攻撃する二つの経路

ビットコインは二つの暗号学体系に依存しており、それぞれに対する量子脅威の水準が異なる：

| 暗号体系 | 用途 | 量子攻撃アルゴリズム | 脅威水準 |
|-----------|------|-------------------|----------|
| **ECDSA (secp256k1)** | 取引署名 (個人キー → 公開キー) | **ショアアルゴリズム** (指数的加速) | **高い** — 公開キーから個人キー逆抽出可能 |
| **SHA-256** | 採掘 (作業証明)、ブロック連結 | **グローバーアルゴリズム** (平方根加速) | **低い** — $2^{256}$が$2^{128}$に減る水準 |

この違いが核心だ。量子コンピュータの脅威は **均一ではない。**

**ショアアルゴリズムとECDSA**：最も深刻な脅威は取引署名体系に対するものだ。現在ビットコインはECDSAを使用して「この取引がビットコイン所有者によって承認された」ことを証明する。ショアアルゴリズムはこの過程を逆転させることができる — 公開キーだけで個人キーを抽出すること。成功すれば **他人のビットコインを自由に使用** できるようになる。

**グローバーアルゴリズムとSHA-256**：採掘に使用されるSHA-256に対してはグローバーアルゴリズムが適用できるが、これは **平方根水準の加速** に過ぎない。$2^{256}$回の試みが$2^{128}$回に減るのだ。$2^{128}$がどれほど大きな数か感覚がないなら — これは **340,282,366,920,938,463,463,374,607,431,768,211,456** だ。依然としてブルートフォースで破るには天文学的な数字だ。SHA-256は量子コンピュータに対しても事実上安全だ。

#### 現実との距離：105キュービット vs 数百万キュービット

ではビットコインのECDSAを破るにはどれほど強力な量子コンピュータが必要か？

```
[量子コンピュータの現在 vs ビットコインを破るのに必要な水準]
...
ECDSA解読に必要な水準:
  約 2,330 論理的キュービット (最小理論値)
  → エラー補正を勘案すれば: 数百万物理的キュービット必要

SHA-256 解読に必要な水準:
  約 250万 論理的キュービット
  → エラー補正を勘案すれば: 数千万~数億物理的キュービット必要

現在のギャップ:
  105 キュービット → 数百万キュービット
  約 10,000倍 ~ 1,000,000倍の格差
```

ここで「論理的キュービット」と「物理的キュービット」の違いが重要だ。量子コンピュータは極度に不安定だ。キュービットは周辺環境の微細な振動、温度変化、電磁波によって容易にエラーが発生する（これを **デコヒーレンス** と言う）。一つの安定的な「論理的キュービット」を作るには数千個の物理的キュービットでエラーを補正しなければならない。

ゲーム開発者ならこんな比喩が心に響くだろう。**物理シミュレーションの精度** を考えてみよう。`float`精度で十分な作業があり、`double`が必要な作業がある。量子コンピュータも同様に、単純な計算（エラーが少しあっても良い）と暗号解読（完璧な正確度が必要な）で要求されるキュービット数が完全に異なる。

2026年2月基準、Iceberg Quantumの **ピナクルアーキテクチャ(Pinnacle Architecture)** がQLDPCコードを使用してRSA-2048解読に必要な物理的キュービットを10万個未満に減らすことに成功した。既存比約10倍の減少だ。量子コンピューティングの効率が急速に改善されているという信号だが、ビットコインを破るにはまだ遠い道のりだ。

Blockstream CEO アダム・バック(Adam Back)は「暗号学的に意味のある量子脅威は **20~40年後** にやっと現実になるだろう」という立場だ。

#### もし量子コンピュータが本当にビットコインを破ったら？

ここでより大きな絵を見なければならない。ビットコインのECDSAを破れる量子コンピュータが登場するということは、**ビットコインだけの問題ではない。**

```
[量子コンピュータがECDSAを破れば一緒に崩れるもの]

✗ ビットコイン、イーサリアムなどすべての暗号通貨
✗ HTTPS (インターネットセキュリティの基盤)
✗ オンラインバンキング (TLS/SSL)
✗ 電子署名 (電子政府、契約、認証書)
✗ VPN、SSH、暗号化メール
✗ 軍事通信暗号化
...
→ 人類文明のデジタルインフラ全体が脅かされる状況
→ この時「ビットコインが危険だ」というのは「家に火がついたのに額縁が斜めになった」と心配する水準
```

したがって量子コンピュータが現実的脅威になる時点には、ビットコインだけでなく **人類のすべてのデジタルセキュリティ体系が量子耐性(Post-Quantum)暗号に転換** しているだろう。そしてビットコインも例外ではない。

#### ビットコインの防衛線：量子耐性暗号

ビットコインは **ソフトフォーク(Soft Fork)** を通じてプロトコルをアップグレードできる。実際に量子耐性署名方式への転換はすでに研究されている：

- **NIST標準化**: 2024年米国国立標準技術研究所(NIST)が量子耐性暗号標準3つを発表 (CRYSTALS-Kyber, CRYSTALS-Dilithium, SPHINCS+)
- **ビットコインコミュニティ**: BIP(Bitcoin Improvement Proposal)を通じた量子耐性署名アルゴリズム導入が議論中
- **ハッシュ基盤署名**: SHA-256自体が量子耐性が高いため、Lamport署名のようなハッシュ基盤署名に転換すれば追加的な安全性確保可能

ビットコインが持つもう一つの防衛メカニズムは **アドレス再使用回避** だ。ビットコインアドレスは公開キーのハッシュだが、取引を送るまでは公開キー自体がネットワークに露出されない。量子コンピュータのショアアルゴリズムは **公開キー** を知らなければ個人キーを抽出できないので、一度も使用していないアドレスに保管されたビットコインは量子コンピュータから安全だ。

つまり、量子脅威に対する現実的評価はこうだ：

| シナリオ | 確率 | ビットコイン影響 |
|----------|------|-------------|
| 10年内暗号学的量子コンピュータ登場 | 非常に低い | 転換時間十分 |
| 20~30年内登場 | 可能性あり | 量子耐性暗号にアップグレード完了予想 |
| 量子コンピュータが突然秘密裏に開発される | 極めて低い | すべてのデジタルセキュリティが同時脅威 |
| 量子コンピュータが永遠に実用化されない | 可能性あり | ビットコインに影響なし |

結論的に、量子コンピュータはビットコインの **理論的脅威** ではあるが **即刻的な危険** ではない。ビットコインは15年間の歴史で数多くの技術的挑戦を克服しながらアップグレードされてきたし、量子脅威に対する備えもすでに進行中だ。ただし、技術の発展速度は予測し難いという点は常に念頭に置く必要がある。
