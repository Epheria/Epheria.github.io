...
<div class="setup-axes" style="margin:1.5rem 0">
  ...
  <p class="sa-cap">この五つの軸がすべて適用された状態で初めて「無人15時間+ 安定 / 自己修復トリガー0件」のメトリクスが出ます。<strong>そのうち5番のヘルスチェックが決定的</strong> — 1〜4番が「最初にきちんと敷いておく」セットアップ軸だとすれば、5番は「敷いたものが壊れた時に復旧する」運用軸であり、第1回の<em>SchTasks Ready 5/5</em>メトリクスに直結します。</p>
</div>
<style>
...
```bash
# 福岡の Mac から
ssh home-laptop "Set-Content C:\Users\Sehyup\tailscale-setup\pending-key.txt '<公開鍵 1行>'; schtasks /Run /TN AddSshKey"
sleep 5
...
> ssh home-laptop          # Tailscale 100.64.x.x で即接続 (正常)
> ```
>
...
**重要なのは「欠陥がないこと」ではなく「欠陥が速く塞がれること」** です。CVE-2022-41924は発見 → 公開セキュリティ勧告 → 自動更新の推奨 → パッチ配布というサイクルを会社が正常に回し、TS-2025-005の鍵ローテーション勧告も同じパターンでした。このサイクルが破綻しない限り — そして本シリーズが自動更新の有効化 + セキュリティ勧告の定期確認 + ACL deny-by-defaultを維持する限り — Tailscaleを信頼モデルの中に置くことができます。
...
| # | 項目 | 検証方法 |
|---|---|---|
| 1 | コンピュータ名が`home-laptop`で適用 | `hostname` |
