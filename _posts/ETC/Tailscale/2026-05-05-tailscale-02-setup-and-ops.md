...
```bash
# 후쿠오카 Mac에서
ssh home-laptop "Set-Content C:\Users\Sehyup\tailscale-setup\pending-key.txt '<공개키 한 줄>'; schtasks /Run /TN AddSshKey"
sleep 5
...
> ssh home-laptop   # Tailscale 100.64.x.x 로 즉시 접속 (정상)
> ```
>
...
| # | 항목 | 검증 방법 |
|---|---|---|
| 1 | 컴퓨터 이름이 `home-laptop`으로 적용 | `hostname` |
...
| 5 | **다른 네트워크에서** SSH 접속 성공 (4G 등) | `ssh home-laptop` |
| 6 | 클램쉘 상태에서 `status.ps1` baseline 캡처 | 위 명령 |
| 7 | `NoAutoRebootWithLoggedOnUsers=1` 적용 | 레지스트리 확인 |
| 8 | **다중화**: 후쿠오카 데스크톱에서도 SSH 가능 | `ssh home-laptop` (다른 머신) |
