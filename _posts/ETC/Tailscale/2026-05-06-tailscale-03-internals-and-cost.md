...
본 시리즈가 `ssh home-laptop` 한 줄로 부산 노트북에 닿을 수 있는 이유가 MagicDNS입니다. tailnet 노드의 **짧은 호스트명을 100.64.x.x로 자동 매핑**하는 기능입니다.
  ...
  <div class="md-flow">
    ...
    <div class="md-step">
      ...
      <div class="md-body">
        ...
        <code>home-laptop</code> 또는 <code>home-laptop.tailnet-xxxx.ts.net</code> 같은 쿼리가 들어오면, 클라이언트가 control plane에서 받아둔 매핑 테이블을 보고 <code>100.64.x.x</code> 응답.
      </div>
    </div>
    <div class="md-step">
    ...
    <div class="md-step">
      ...
      <div class="md-body">
        ...
        <code>ssh home-laptop</code> 같이 짧은 이름을 치면 OS가 자동으로 <code>.tailnet-xxxx.ts.net</code>을 붙여서 쿼리 — 그래서 짧은 이름으로 닿는다.
      </div>
    </div>
  </div>
</div>
...
핵심은 **Tailscale 클라이언트가 OS의 DNS를 가로채되 일반 트래픽은 흘려보낸다**는 split-horizon DNS 모델입니다. 사용자는 별도 설정 없이 `ssh home-laptop`을 칠 수 있고, 동시에 일반 웹 브라우징도 영향받지 않습니다.
