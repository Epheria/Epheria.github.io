...
<div class="candidates" style="margin:1.5rem 0">
  <div class="cd-grid">
    <div class="cd-card cd-1">
      ...
      <div class="cd-detail">同一ネットワークなら直接通信。NAT越え不要。</div>
    </div>
    <div class="cd-card cd-2">
      <div class="cd-num">2</div>
      <div class="cd-name">STUN WANアドレス</div>
      <div class="cd-detail">EIM NATでSTUNにより発見した public:port</div>
    </div>
    ...
    <div class="cd-card cd-4">
      ...
      <div class="cd-detail">IPv6-only環境専用</div>
    </div>
    <div class="cd-card cd-5">
      <div class="cd-num">5</div>
      <div class="cd-name">DERP relay</div>
      <div class="cd-detail">他の4つが失敗しても動作。<strong>常にpreselected</strong> — 最初のパケットからDERP上で通信開始、速い経路が見つかれば即座にアップグレード。UDPが塞がれた環境でも保証。</div>
    </div>
  </div>
...
**より厳しいケース — 両端ともEDM**: 探索空間が掛け算になり、99.9%到達まで約28分かかります(170,000 probes / 100 pkt/s)。両端ともEDMのケースでは事実上DERP fallbackに落ち込みます。
...
本シリーズで `ssh home-laptop` の一行で釜山のノートPCに辿り着けるのはMagicDNSのおかげです。tailnetノードの **短いホスト名を100.64.x.xに自動マッピング** する機能です。
  ...
  <div class="md-flow">
    ...
    <div class="md-step">
      ...
      <div class="md-body">
        ...
        <code>home-laptop</code> あるいは <code>home-laptop.tailnet-xxxx.ts.net</code> のようなクエリが来ると、クライアントがcontrol planeから受け取っておいたマッピングテーブルを参照して <code>100.64.x.x</code> を応答。
      </div>
    </div>
    <div class="md-step">
    ...
    <div class="md-step">
      ...
      <div class="md-body">
        ...
        <code>ssh home-laptop</code> のように短い名前を打つと、OSが自動的に <code>.tailnet-xxxx.ts.net</code> を補ってクエリ — だから短い名前で届く。
      </div>
    </div>
  </div>
</div>
...
要点は、 **TailscaleクライアントがOSのDNSを横取りしつつ一般トラフィックは流す** というsplit-horizon DNSモデルです。ユーザーは設定なしで `ssh home-laptop` と打てて、同時に普段のWebブラウジングにも影響しません。
...
この8つは1つのクライアント内部で同時に動作しますが、役割は同じではありません。**データプレーン(WireGuard)が全通信の基盤** であり、残りの7つはすべて「WireGuardパケットをどこに通すか」を解く道具です。そのうち **DERPは他のすべてが失敗しても最初のパケットから通信を開始** させるセーフティネット — 別モードが用意されているのではなく、**すべての接続が開始時点でDERPの上を流れ**、より速い経路(LAN/STUN/ポートマップ/hole punching)が見つかれば即座にデータプレーンをアップグレードします。

よくある誤解 — 「P2P directができなければTailscaleは使えない」は間違いです。本シリーズは両側のルーターがいずれもEIMだったためP2Pが成立しましたが、両端ともEDMでbirthday paradoxに28分かかる環境でもDERP relay経由で通信はそのまま可能です(速度が落ちるだけ)。

ユーザー側からすれば `tailscale up` の一行とadminコンソールのexit node承認1回ですが、その一行の裏で8つの道具が競争・協力していることが、本シリーズが描きたかった全体像です。
