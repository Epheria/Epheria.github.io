...
- **Prompt Caching**: 원칙은 Stateless이지만, 최근 Anthropic(Claude)과 OpenAI 등은 비용과 지연 시간을 줄이기 위해 **프롬프트 캐싱(Prompt Caching)** 기술을 지원합니다. 클라이언트가 같은 긴 컨텍스트(예: CLAUDE.md, 시스템 프롬프트)를 반복 전송할 때, 서버가 이전 연산 결과(KV Cache)를 임시로 보존하여 중복 연산을 생략합니다. 이는 "기억"이 아니라 **연산 캐시의 재활용**이므로 Stateless 원칙과 모순되지 않습니다 (섹션 12에서 상세 설명).
...
왜 시스템 RAM이 아닌 VRAM이 중요한가? **대역폭** 때문입니다. GPU가 데이터를 처리하려면 메모리에서 읽어와야 하는데, VRAM의 대역폭은 시스템 RAM 대비 수십 배 빠릅니다 (GPU 종류에 따라 차이가 큼).
...
| 구성 요소 | 특성 | 설명 | 70B 모델 (FP16) 기준 |
| --- | --- | --- | --- |
| 모델 가중치 | **정적** | 학습된 파라미터 전체 (서버 시작 시 로드, 변하지 않음) | ~140 GB |
| KV 캐시 | **동적** | 이전 토큰의 Key/Value (대화 길이에 비례하여 **런타임에 증가**) | ~수 GB~수십 GB |
| 활성화 메모리 | **동적** | 순전파 중간 연산 결과 | ~수 GB |

> **게임 개발자를 위한 비유**: 모델 가중치는 게임의 **정적 에셋**(텍스처, 메시 등 빌드 시 확정)이고, KV 캐시는 **동적 프레임 버퍼나 런타임 상태 저장소**(플레이 중 계속 쌓이는 데이터)에 해당합니다. 대화(컨텍스트)가 길어질수록 KV 캐시가 동적으로 증가하여 VRAM을 추가로 점유하므로, 실제 서빙 시에는 가중치 크기만으로 VRAM 요구량을 판단할 수 없습니다. 이것이 실사용 가능한 모델 규모가 가중치 기준 추정치보다 작아지는 핵심 이유입니다.

VRAM이 부족하면 모델의 일부를 시스템 RAM으로 내리는 **레이어 오프로딩(Layer Offloading)**이 발생하고, 추론 속도가 급격히 저하됩니다. 이것은 게임 엔진이 비디오 메모리가 부족할 때 시스템 메모리로 텍스처를 페이징(Paging)하여 프레임이 급락하는 현상과 비슷합니다. llama.cpp 같은 로컬 LLM 도구에서는 GPU에 적재할 레이어 수를 직접 지정하여 GPU/CPU 분담 비율을 조절할 수 있습니다. 양자화(INT4/INT8)로 가중치 크기를 줄이는 것이 오프로딩을 방지하는 주된 방법입니다.
...
| 대역폭 | HBM 전용 (~3.3TB/s, H100 기준) | SoC 통합 메모리 공유 (~546GB/s, M4 Max 기준)* |
| 용도 | 서버, 학습, 대형 모델 | 모바일, 경량 모델 |

*Apple Silicon의 통합 메모리 대역폭은 칩에 따라 큰 차이가 있습니다: M4 Max ~546GB/s, M4 Ultra ~800+GB/s. 이 대역폭을 CPU·GPU·NPU가 공유합니다. GPU의 3.3TB/s는 HBM 전용 대역폭이므로 직접 비교 시 주의가 필요합니다.
...
| --- | --- | --- |
| NVIDIA RTX 4090 | 24GB VRAM | ~25-35B (4bit 양자화, 컨텍스트 길이에 따라 다름)* |
| NVIDIA RTX 5090 | 32GB VRAM | ~35-50B (4bit 양자화, 컨텍스트 길이에 따라 다름)* |
| M4 MacBook Pro | 24GB 통합 메모리 | ~25-35B (4bit 양자화, 컨텍스트 길이에 따라 다름)* |
| M4 Max MacBook Pro | 64~128GB 통합 메모리 | ~70B (4bit 양자화) |
| Mac Studio M2/M4 Ultra | 192~512GB 통합 메모리 | ~200B+ (4bit 양자화) |

*모델 가중치만으로는 더 큰 모델이 적재 가능하지만, 추론 시 KV Cache(대화 길이에 비례하여 증가하는 동적 메모리)가 VRAM을 추가로 점유하므로 실사용 가능한 모델 규모가 줄어듭니다. 짧은 컨텍스트에서는 위 추정치보다 더 큰 모델도 가능합니다.
...
| --- | --- | --- |
| 메모리 대역폭 | ~1,008GB/s (GDDR6X) | ~546GB/s |
...
초기에는 **Chinchilla 법칙**이 지배적이었습니다. "주어진 연산 예산에서 모델 크기와 학습 데이터 양의 최적 비율(compute-optimal ratio)을 맞춰야 한다"는 규칙이죠. 하지만 2025년에는 이 최적 비율을 의도적으로 넘어서는 **over-training 전략**이 주류가 되었습니다. Llama 시리즈가 대표적인데, 8B 파라미터의 작은 모델이라도 Chinchilla 최적 비율보다 훨씬 많은 수조 개의 토큰으로 과도하게 학습시킵니다. 학습 시 연산 효율은 떨어지지만, **추론 시 작은 모델을 사용하므로 서빙 비용이 크게 절감**됩니다. 결과적으로 이전 세대의 훨씬 큰 모델보다 뛰어난 성능을 보이면서도 추론은 훨씬 저렴해지는 효과를 얻습니다.
...
렌더링 최적화에 익숙한 개발자라면, MoE를 **프러스텀 컬링(Frustum Culling)**과 비교하면 더 직관적입니다. 게임에서 전체 씬의 메시 데이터가 메모리에 올라가 있지만, 카메라 시야에 없는 객체의 렌더링 연산을 생략하는 것처럼, MoE 모델도 전체 가중치가 VRAM에 상주하지만 **하나의 토큰을 생성할 때는 라우터가 선택한 일부 전문가(Expert)만 연산에 참여**합니다. 메모리는 전체를 차지하되, 연산량은 일부만 사용하는 것입니다.
...
최신 기법인 **DFlash**는 블록 확산(Block Diffusion) 모델을 도입해 초안 생성 자체를 병렬화함으로써 기존 투기적 디코딩보다 **2.5배 이상의 추가 속도 향상**을 보고했습니다 ([arXiv:2602.06036](https://arxiv.org/html/2602.06036v1), 프리프린트 기준, 독립 재현 검증 필요).
...
| 질문: "이 심볼의 타입은?" | 반환 데이터 | 상대적 토큰 규모 |
| --- | --- | --- |
| **Read** (파일 전체 읽기) | 500줄 전체 | ██████████ 가장 큼 (파일 크기에 비례) |
| **Grep** (패턴 매칭) | 매칭 라인들 | ███░░░░░░░ 중간 (매칭 결과 수에 비례) |
| **LSP** (의미적 질의) | 타입/시그니처만 | █░░░░░░░░░ 최소 (필요한 정보만 반환) |

> **참고**: 위 비교는 상대적 크기 비율을 나타냅니다. 실제 토큰 수는 파일 크기, 검색 결과 수, 언어에 따라 크게 변동합니다. 핵심은 LSP/MCP가 Read/Grep 대비 **수십~수백 배 효율적**이라는 방향성입니다.
...
> **📅 역사적 참고용 비교**: 아래 비교는 **2024년 하반기 플래그십 모델**(GPT-4, Claude 3.5, Gemini 1.5) 기준으로 작성되었으며, **각 모델의 설계 철학과 강점 영역을 이해하기 위한 역사적 참고 자료**입니다. 2025~2026년 현재는 GPT-4o/o3/GPT-5, Claude 4.5/4.6, Gemini 2.0/2.5 Pro 등 후속 모델이 출시되어 컨텍스트 윈도우(Claude 4.6은 1M 베타), 성능, 가격이 크게 변동되었습니다. **현재 성능 비교에는 적합하지 않으며**, 최신 정보는 각 공식 문서를 참조하세요.
...
Constitutional AI는 **학습(Training) 단계**에서 모델이 헌법적 원칙(Constitutional Principles)에 따라 자체 비평하도록 훈련하는 기법입니다. (1) 모델이 응답을 생성하고 (2) 정해진 원칙 기준으로 자체 비평한 뒤 (3) 수정된 응답을 생성합니다. 이렇게 만들어진 데이터로 RLAIF(RL from AI Feedback) 학습을 수행합니다. 추론 시에는 이미 학습된 결과가 반영되어 있으므로, 별도의 자체 평가 과정 없이 안전한 응답을 생성합니다.
...
지금까지 LLM의 원리와 최적화를 살펴봤다면, 이제 가장 실전적인 질문에 답해봅시다: **게임 엔진 안에서 신경망을 어떻게 실행할 수 있는가?** Unity와 Unreal Engine 모두 신경망을 엔진 내부에서 실행하기 위한 도구를 제공하고 있습니다.

> **⚠️ 중요한 구분**: 이 파트에서 다루는 Unity Sentis와 Unreal NNE는 **경량 추론 모델**(제스처 인식, NPC 행동 패턴 예측, 이미지 분류 등 수 MB~수백 MB 규모)에 최적화되어 있습니다. Claude, GPT-4 같은 **대형 LLM(수 GB~수백 GB)**을 게임 런타임에서 직접 구동하는 것과는 다릅니다. 대형 LLM이 필요한 경우(NPC 대화 생성 등)는 **클라우드 API 호출**을 조합하는 것이 현실적입니다.
...
AI 에이전트가 플레이어의 숙련도, 플레이 스타일, 감정 상태를 실시간으로 트래킹하여 게임의 난이도와 내러티브 분기를 실시간으로 조정하는 시스템이 연구되고 있습니다. 일부 업계 보고에서는 **플레이어 유지율 향상**, **콘텐츠 제작 시간 단축** 등의 성과를 언급하고 있으나, 구체적인 수치는 프로젝트와 구현 방식에 따라 크게 다를 수 있습니다. (참고: [DigitalDefynd - Agentic AI in Gaming](https://digitaldefynd.com/IQ/agentic-ai-in-gaming/))
...
| **SPEQ: Speculative Decoding via Quantization** | 양자화 기반 투기적 디코딩 | [arXiv](https://arxiv.org/abs/2502.12759) |
