---
title: Machine Learning - Feature Scaling & Learning Rate
date: 2024-06-12 10:00:00 +0900
categories: [ML, Supervised Machine Learning]
tags: [Machine Learning, ML, feature scaling, learning rate]
...
mermaid: true
tldr:
  - "학습률(α)이 너무 크면 Overshooting이 발생해 비용 함수가 발산하고, 너무 작으면 수렴이 느리다"
  - "Feature Scaling(정규화)으로 특성 범위를 맞추면 경사 하강법이 훨씬 빠르게 수렴한다"
  - "Z-score normalization: (x - mean) / std 로 평균 0, 표준편차 1인 분포로 변환한다"
  - "다중 선형 회귀에서 비용 함수 최소화는 각 w_j에 대해 편미분을 동시에 업데이트하여 진행한다"
---
...
> [Feature Scaling - 특성 스케일링](#feature-scaling---특성-스케일링)     
> [실습 - Learning Rate](#실습---learning-rate)
...
- 다중 선형 회귀에서 각 특성(feature)의 범위가 크게 다를 때, 경사 하강법의 수렴 속도가 매우 느려질 수 있다.
- 예를 들어, 주택 크기는 300 ~ 10000 sqft 범위이고, 침실 수는 1 ~ 5 범위라면, 비용 함수 $J(\vec{w}, b)$ 의 등고선은 매우 길쭉한 타원형이 된다.
- 이렇게 되면 경사 하강법이 지그재그로 진행되어 수렴이 느려진다.

<br>

#### 왜 Feature Scaling 이 필요한가?

- 특성값의 범위가 서로 다르면 매개변수 $w_j$ 가 받는 기울기(gradient) 크기도 크게 달라진다.
- 범위가 큰 특성의 $w_j$ 는 작은 값으로도 예측에 큰 영향을 미치고, 범위가 작은 특성의 $w_j$ 는 큰 값이 필요하다.
- 이 불균형 때문에 경사 하강법이 비효율적인 경로를 따라 수렴하게 된다.

<br>

#### Feature Scaling 방법들

**1. Mean Normalization (평균 정규화)**

$$ \large{x_j = \frac{x_j - \mu_j}{\max(x_j) - \min(x_j)}} $$

- $\mu_j$ 는 특성 $j$ 의 평균값이다.
- 결과값은 대략 $-1 \leq x_j \leq 1$ 범위를 갖는다.
...
- $\mu_j$ 는 특성 $j$ 의 평균, $\sigma_j$ 는 표준편차이다.
- 결과값은 평균 0, 표준편차 1인 분포로 변환된다.
- 가장 일반적으로 사용되는 방법이다.
...
def zscore_normalize_features(X):
    ...
    Returns:
      X_norm (ndarray (m,n)): normalized X
...
$$ \large{x_j = \frac{x_j - \min(x_j)}{\max(x_j) - \min(x_j)}} $$

- 결과값은 $0 \leq x_j \leq 1$ 범위가 된다.

<br>

#### Feature Scaling 적용 기준

- 일반적으로 특성의 범위가 $-1 \leq x_j \leq 1$ 근처가 아닌 경우 Feature Scaling 을 적용하는 것이 좋다.
- 다음과 같은 경우 스케일링을 고려해야 한다.
...
| $-3 \leq x_j \leq 3$ | ✅ 적절함 |
| $0 \leq x_j \leq 3$ | ✅ 적절함 |
| $-100 \leq x_j \leq 100$ | ❌ 스케일링 필요 |
| $0.001 \leq x_j \leq 0.01$ | ❌ 스케일링 필요 |
| $98.6 \leq x_j \leq 105$ | ❌ 스케일링 필요 |

<br>

#### Convergence 확인 방법

- 경사 하강법이 잘 수렴하고 있는지 확인하려면 반복 횟수에 따른 비용 $J$ 의 변화를 관찰한다.
- 매 반복마다 $J$ 가 감소해야 한다.
- $J$ 가 더 이상 유의미하게 감소하지 않으면 수렴했다고 판단할 수 있다.
- 자동으로 수렴을 판단하는 방법으로는 $\epsilon = 10^{-3}$ 같은 작은 값을 설정하여, $J$ 가 한 반복에서 $\epsilon$ 보다 적게 감소하면 수렴으로 판단하는 방법이 있다.
...
- 그래디언트 값은 반복이 진행됨에 따라 점차 감소하고 있다. 이는 모델이 점점 최적화되고 있으며, 경사 하강법 알고리즘이 수렴하고 있음을 나타낸다.
