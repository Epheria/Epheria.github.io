...
tags: [Machine Learning, ML, supervised, cost function]
...
<div class="chart-wrapper">
  <div class="chart-title">J(w) — w 값에 따른 비용 함수 변화 (훈련 데이터: (1,1) (2,2) (3,3))</div>
  <canvas id="costFnChart" class="chart-canvas" height="220"></canvas>
</div>

<script>
window.chartConfigs = window.chartConfigs || [];
(function() {
  // J(w) = 1/(2*3) * sum((w*x - y)^2) for data (1,1),(2,2),(3,3)
  var labels = [], data = [];
  for (var w = -0.5; w <= 2.5; w += 0.1) {
    var j = (Math.pow(w*1-1,2) + Math.pow(w*2-2,2) + Math.pow(w*3-3,2)) / 6;
    labels.push(Math.round(w*10)/10);
    data.push(Math.round(j*1000)/1000);
  }
  window.chartConfigs.push({
    ...
    data: {
      ...
      datasets: [{
        ...
        label: '최솟값 (w=1, J=0)',
        data: labels.map(function(l) { return Math.abs(l - 1) < 0.05 ? 0 : null; }),
    ...
    options: {
      ...
      scales: {
        x: { title: { display: true, text: 'w (기울기 파라미터)' } },
        y: { beginAtZero: true, title: { display: true, text: 'J(w) 비용' } }
      }
    }
  });
})();
