...
categories: [Csharp, Data Structure]
...
mermaid: true
tldr:
  - "Dictionary는 O(1) 해시 테이블 기반으로, 게임 개발에서 가장 많이 사용되는 Key-Value 자료구조다"
  - "SortedDictionary는 O(log n) 레드-블랙 트리 기반으로, 삽입/삭제 시에도 항상 정렬 상태를 유지한다"
  - "SortedList는 삽입이 느리지만(O(n)) 메모리 효율적이며 인덱스(배열처럼)로 접근 가능하다"
  - "성능 우선이면 Dictionary, 정렬 필요하면 SortedDictionary, 메모리 절약+인덱스 접근이면 SortedList"
---
...
<div class="code-compare">
  <div class="code-compare-pane">
    <div class="code-compare-label label-before">Before — 용량 미지정</div>
    <div class="highlight">
<pre><code class="language-csharp">// 리사이징이 여러 번 발생할 수 있음
...
foreach (var data in allMonsterData)
{
  ...
  <div class="code-compare-pane">
    <div class="code-compare-label label-after">After — 초기 용량 지정</div>
    <div class="highlight">
<pre><code class="language-csharp">// 리사이징 0회 — O(n) 비용 절약
