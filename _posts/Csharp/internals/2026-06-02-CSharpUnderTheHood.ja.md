...
だから `Reflection.Emit` がランタイムで IL をいくら上手に作り出しても、それを機械語に変えて実行する場所がありません。コードが壊れます。これは IL2CPP と NativeAOT が**共有する**制約です（どちらも AOT だからです）。[Foundation 第 3 回](/posts/DotnetRuntimeVariants/) で表にまとめた「AOT で壊れるもの」の根本原因が、まさにこの一行でした。
