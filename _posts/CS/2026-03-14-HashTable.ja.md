---
title: "CSロードマップ (3) — ハッシュテーブル：O(1)の魔法とその代償"
lang: ja
...
tldr:
  ...
  - "ロードファクター（α）が0.7を超えると衝突確率が急増し、性能がO(1)からO(n)に退化する。適切なタイミングでのリサイズがハッシュテーブル性能の鍵である"
...
乗算法の利点はmを自由に選択できることだ。特に **mを2の冪乗** に選べばモジュラー演算をビットシフトに置き換えられるため高速になる。
...
| **MurmurHash3** | 非常に高速 | 良好 | 汎用ハッシュテーブル、Redis |
| **xxHash** | 極めて高速 | 良好 | チェックサム、データ処理 |
| **SipHash** | 普通 | 良好 + DoS防御 | Rust HashMap、Python dict |
| **wyhash** | 極めて高速 | 良好 | Go map (1.17+) |
...
**α = 0.7を超えると性能が急激に悪化する。** 特に失敗探索（キーが存在しない場合）は空きスロットに出会うまで探索を続けなければならないため、より敏感だ。これがほとんどのハッシュテーブルがロードファクター **0.7 ~ 0.75** の閾値でリサイズを行う理由である。
...
| Go `map` | **6.5** (バケットあたり) | チェイニング (8スロット) | 2倍 |
| Rust `HashMap` | **0.875** | Robin Hood → SwissTable | 2倍 |

Javaは特殊で、一つのバケットに要素が**8個以上**積み重なると連結リストを**レッドブラックツリー**に変換する。O(n)への退化をO(log n)で防ぐ防御メカニズムだ。
...
1. **メタデータ配列(control bytes)**：各スロットの状態（空き/使用中/削除済み）とハッシュの上位7ビットを**1バイトに圧縮**して別の配列に格納
2. **SIMD探索**：16個のメタデータをSSE2命令**1回**で同時比較

```
Control bytes (16個を一度に比較):
[0x82][0xFF][0x31][0xFF][0x82][0x55][0xFF][0xFF]
[0xFF][0x82][0xFF][0xFF][0xFF][0xFF][0xFF][0xFF]
  ↑ 0x80以上 = 使用中、下位7ビット = ハッシュ上位7ビット
  ↑ 0xFF = 空き
  ↑ 0xFE = 削除済み (tombstone)

キー "オーク"のハッシュ上位7ビット = 0x31の場合：
SIMDで16バイトを一度に比較 → index 2でマッチ！
→ 実際のキー比較は1回だけ
```

**結果**：キャッシュライン1本(64バイト)にメタデータ64個が収まる。ほとんどの探索がSIMD1回 + キー比較1回で完了する。

この設計の影響：
- C++ Abseil `absl::flat_hash_map`
- Rust `hashbrown` (SwissTableポート → 標準HashMap)
- Go 1.21+ `runtime.map` (Swiss Tables導入の議論)
...
3. **ロードファクター（α）が性能の核心的な調整器**だ。αが0.7を超えると衝突が急増し、0.9を超えると事実上使用不能になる。リサイズはO(n)だが償却O(1)だ — それでもゲームでは初期容量設定で完全に防ぐのが最善だ。
