...
위에서 설명한 Plugin을 설치를 했다면, 다음과 같이 작성하면 된다.
   ...
   project_path: "/path/to/your/jenkins/workspace/android_fastlane" # Unity Project 경로 입력
   )
   ```
<br>
   ...
   # appcenter_upload
   appcenter_upload(
      api_token: "YOUR_APPCENTER_API_TOKEN", # 본인의 appcenter - settings - API Token 을 입력
      owner_name: "YOUR_OWNER_NAME", # appcenter 관리자 이름 입력 (All apps - Owner 정보 나와있음)
      app_name: "toyverse_alpha_android", # 앱 이름 (유니티 이름 x 앱센터 상의 이름)
      file: "/path/to/your/build/output.aab", # 최종 결과물의 경로 입력  .apk / .aab 확장자 명까지 같이 입력
