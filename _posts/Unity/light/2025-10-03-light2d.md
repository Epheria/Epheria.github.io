...
protected void RefreshChunks(Vector3 targetPosition, bool isInitialLoad)
{
    ...
    HashSet<Vector2Int> neededChunks = CalculateNeededChunks(targetPosition);

    // 2. 새로 진입한 청크: 이펙트 스폰
    foreach (var chunk in neededChunks)
    {
        if (!activeChunks.Contains(chunk))
            OnChunkEnter(chunk, targetPosition);
    }

    // 3. 이탈한 청크: 처리 (Static/Moving에서 오버라이드)
...
**핵심 설계**: `OnChunkEnter`와 `OnChunkExit`를 가상 메서드로 두어, Static과 Moving이 각자의 전략으로 스폰/반납을 처리합니다.
...
**오버랩 방지**: Moving 이펙트는 스폰 시 `minSeparation` 거리를 체크하여 이펙트끼리 겹치지 않도록 합니다. 최대 `rejectionTry`회까지 재시도하여 적절한 위치를 찾습니다.
...
>
> `SetActive(false)`를 하면 다시 켤 때 Unity 내부에서 **Awake/OnEnable 콜백 재실행**, 물리/렌더 재등록 등 비용이 발생합니다. 반면 Light2D의 intensity나 enabled 토글은 **렌더 파이프라인 레벨**에서만 동작하여 훨씬 가볍습니다. 수십 개의 이펙트를 매 프레임 토글해야 하는 상황에서 이 차이는 체감됩니다.
>
...
flowchart TB
    ...
    subgraph Component["PooledGameObject (컴포넌트)"]
        ...
        C2["위치를 (10000, 10000, 10000)으로 이동"]
        C3["다음 프레임에 비활성화"]
        C1 --> C2 --> C3
    end
...
stateDiagram-v2
    ...
    Teleport --> Pool: (10000,10000,10000)으로 이동 후 비활성화
    Pool --> Active: 재사용 Get()
    Pool --> [*]: CleanUp (Destroy)
```

**왜 즉시 `SetActive(false)` 하지 않고 먼 곳으로 텔레포트하는가?** → 같은 프레임에 `Release()`와 `SetActive(false)`가 동시에 일어나면, Light2D 렌더 패스에서 **한 프레임 깜빡임**이 발생할 수 있습니다. 먼저 화면 밖으로 이동시킨 뒤 비활성화하면 이 문제를 방지합니다.
...
{
    public Vector3 BaseScale { get; private set; }           // 원본 스케일 보존
    public Light2D[] CachedLights { get; private set; }      // Light2D 캐시
    public ParallaxEffectType Type;                           // Static / Moving 구분

    public void EnsureCache(bool forceRefresh = false)
    {
        if (CachedLights == null || forceRefresh)
        {
            CachedLights = GetComponentsInChildren<Light2D>(true);
...
public void TransitionLightColors(Light2DSetting newSetting, float duration)
{
    ...
    {
        foreach (var light in tag.CachedLights)
        {
...
> **설계 포인트**: 씬이 파괴될 때 진행 중인 DOTween이 이미 파괴된 오브젝트에 접근하면 크래시가 발생합니다. `OnDestroy`에서 반드시 `DOTween.Kill()`로 진행 중인 트윈을 정리합니다.
{: .prompt-warning }
...
SRDebugger의 Options Panel에 `Light2DSetting`이 **자동 등록**됩니다. 트리플 탭으로 디버그 패널을 열어 런타임 중 라이트 색상/강도/스케일을 자유롭게 튜닝할 수 있습니다. 변경 사항은 `OnSettingChanged` 이벤트를 통해 시스템 전체에 브로드캐스트됩니다.
...
| **텔레포트 후 비활성화** | PooledGameObject.Release | 1프레임 깜빡임 **방지** | (10000,10000,10000) 이동 후 비활성화 |
...
3D 씬에서 Light2D를 사용하는 것은 비정통적인 접근이지만, 청크 기반 Parallax + 오브젝트 풀과 결합하면 **Directional Light + Light Probe 대비 GPU 비용을 크게 절감**하면서도 충분한 시각 품질을 달성할 수 있었습니다. 특히 모바일 환경에서 이 비용 차이는 결정적입니다.

> **💬 잠깐, 이건 알고 가자**
>
> **Q. 이 시스템의 한계는 뭔가요?**
>
> Light2D는 **2D 렌더 패스**에서 동작하므로, 3D 메시에 직접적인 노멀 맵 기반 라이팅(Lambertian 등)을 적용할 수 없습니다. 본 프로젝트에서는 카메라가 탑다운 시점이라 2D 합성으로 충분했지만, 자유시점 3D 게임에서는 다른 전략이 필요합니다. 또한 Light2D 간 그림자 캐스팅이 제한적이므로, 복잡한 그림자 연출이 필요한 경우 보조 기법(Sprite Mask 등)을 병용해야 합니다.
>
> **Q. 확장하려면 어떻게 하나요?**
>
