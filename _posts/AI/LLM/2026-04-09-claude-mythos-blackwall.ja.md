...
2026年4月7日、Anthropicは[Claude Mythos Preview System Card](https://assets.anthropic.com/m/785e231869ea8b3b/original/Claude-Mythos-Preview-System-Card.pdf)（244ページ）とともに[Project Glasswing](https://www.anthropic.com/glasswing)を発表しました。Mythosはanthropicの最新フロンティアモデルであり、**一般公開しないことを決定した初めてのモデル**でもあります。

公開しない理由はシンプルです。サイバーセキュリティ能力が強力すぎるため、防御には有用ですが攻撃に悪用される可能性があるからです。代わりに、選別されたパートナーにのみ防御目的で提供する**Project Glasswing**を通じて展開されています。

サイバーパンク2077をプレイしたことがある方なら、この構造が**ブラックウォール（Blackwall）**と**ネットウォッチ（NetWatch）**を思い起こさせるかもしれません。危険すぎて隔離されたAIを、防御目的のみで管理下に活用する構造。実際に知人が「ブラックウォールAI」と表現していましたが、System Cardを読んでみると、なかなか的確な比喩だと感じました。

この記事では、System CardとGlasswingブログの核心的な内容を整理します。

---

## Part 1: Mythosのサイバー能力
...
ベンチマーク以外にも、Mythosは実際のソフトウェアでゼロデイ脆弱性を自律的に発見し、PoC（Proof-of-Concept）exploitまで作成する能力を示しました。
...
## Part 2: Alignment — 最もよく整合された、最も危険なモデル
...
## Part 3: Project Glasswing — 防御専用配布

このような能力を持つモデルをどのように配布するのか？Anthropicの答えは**防御専用の限定配布**です。
...
サイバーパンク2077では、ブラックウォールには最終的にひびが入ります。現実のブラックウォールがどれだけの間もちこたえられるかは、Mythos以降に登場するモデルたちが決めることになるでしょう。
